...
func NewGroupTopicConsumer(gtConf GroupTopicConfig) (*GroupTopicConsumer, error) {
	...
	if err != nil {
		log.Errorf("[NewGroupTopicConsumer] 连接kafka [%v] 失败:%s", gtConf.Kafka, err.Error())
		return nil, err
	}
	...
	if err != nil {
		log.Errorf("[NewGroupTopicConsumer] 连接zookeeper [%v] 失败:%s", gtConf.Kafka, err.Error())
		return nil, err
	}
...
func (this *GroupTopicConsumer) startPartitionConsumer(partMapThread map[int]int) {
	if len(this.partitionConsumers) > 0 {
		log.Errorf("%s 试图重复启动 partitionConsumer", this.name())
		return
	}
	...
	for partitionNum, threadNum := range partMapThread {
		...
		if err != nil {
			log.Errorf("%s 放弃占用 partition [%d] ", this.name(), partitionNum)
			continue
		}
...
func (this *GroupTopicConsumer) rebalance(topic string) (assigned map[int]int, err error) {
	...
	if err != nil {
		log.Errorf("%s 获取 Topic [%s] Partition 失败:%s ", this.name(), topic, err.Error())
		return
	}
	...
	for clientPosition, client := range clients {
		...
		for i, thread := range threadOfClient {
			...
			for j := 0; j < np; j++ {
				log.Debugf("%s Topic [%s] Partition [%d] 被 分配给了 Client [%s] Thread[%s]", this.name(), topic, partitions[sp+j], client, thread)
	...
	log.Debugf("%s 分配结果:%v", this.name(), assigned)
...
func (this *GroupTopicConsumer) occupyPartition(clientId string, threadNum int, topic string, partition int) (err error) {
	...
	if i == MAX_ZK_CONSUMER_OCCUPY_RETRY {
		log.Errorf("%s 占用 partition [%d] 失败，一直有其他节点节点占用", this.name(), partition)
	}
	return
}
...
func (this *GroupTopicConsumer) zkHeartBeat(closeChan, doneChan chan string) {
	for {
		select {
		case <-closeChan:
			log.Debugf("%s 停止 zk 心跳", this.name())
			doneChan <- "EXITDONE"
			return
		case <-time.After(time.Second * 10):
			// log.Debugf("%s zk 心跳", this.name())
			this.zkConn.Exists("/")
		}
	}
}
func (this *GroupTopicConsumer) registerConsumerInZK(clientId string, subscription map[string]int) (err error) {
	...
	log.Debug("注册clientId：" + idsDir + "/" + clientId)
	return
}
...
func (this *GroupTopicConsumer) watchZkConsumerChange(exitChan, doneChan, firstWatch chan string) (err error) {
	...
	if err != nil {
		log.Errorf("监听 consumer [%s] 变化失败:%s", p, err.Error())
		return
	}
	log.Debugf("监听 consumer [%s]变化", p)
	...
	for {
		select {
		case e := <-childrenChan:
			log.Debugf("监听到了 consumer [%s]变化", p)
			...
			for {
				...
				log.Errorf("监听 consumer 变化失败:%s", err.Error())
				time.Sleep(time.Second * 10)
			}
			this.zkConsumerChan <- e
		case <-exitChan:
			log.Debugf("停止监听 consumer 变化")
			doneChan <- "DONE"
			return
		}
	}
...
func (this *GroupTopicConsumer) Close() {
	...
	case <-time.After(time.Second * 10):
		log.Errorf("[%s] 无法正常退出，强行退出", this.name())
	}
	this.running = false
}
...
func (this *partitionConsumer) run() (err error) {
	...
	if err != nil {
		log.Errorf("%s 获取offset失败，退出run:%s", this.name(), err.Error())
	...
	if err != nil {
		...
		if err != nil {
			...
			log.Errorf("%s 获取KafkaConsumer [topic:%s, partition:%d, requestOffset:%d, highWater:%d]失败，退出run: %s",
	...
	for {
		...
		case msg, ok = <-this.in:
			...
			// log.Debugf("%s 读取到一条消息:[%s],offset:[%d]", this.name(), string(msg.Value), msg.Offset)
...
func (this *partitionConsumer) exit() {
	...
	if err != nil {
		log.Errorf("%s 关闭 kafka 失败：%s", this.name(), err.Error())
	}
	this.statusLock.Unlock()
	if this.running {
		<-this.exitDoneChan
	}
	log.Debugf("%s 成功退出", this.name())
}

func (this *partitionConsumer) updateOffset(offset int64) (savedOffset int64) {
	log.Debugf("%s 更新offset [%d] ", this.name(), offset)
	if err := this.setOffset(this.topic, this.partitionNum, offset); err != nil {
		log.Errorf("%s 更新Offset 失败:%s", this.name(), err.Error())
	}
	return offset
}

func (this *partitionConsumer) setOffset(topic string, partition int, offset int64) (err error) {
	dirPaths := []string{this.zkChroot, "consumers", this.group, "offsets", topic}
	partitionDir := strings.Join(dirPaths, "/")
	...
	if err == zk.ErrNoNode {
		_, err = mkZkDirIfNotExists(this.zkConn, dirPaths...)
		if err != nil {
			return
		}
	...
	if err == nil {
		log.Debugf("成功更新 Topic [%s] Partition [%d] Offset [%d]", topic, partition, offset)
	} else {
		log.Errorf("更新 Topic [%s] Partition [%d] Offset [%d] 失败:%v", topic, partition, offset, err)
	}
	return
}
...
func (this *partitionConsumer) getOffset(topic string, partition int) (offset int64, err error) {
	...
	log.Debugf("成功获取 Topic [%s] Partition [%d] Offset [%d]", topic, partition, offset)
	return
}
